...
1)  How does pilot data influence sample size suggestions?
...
```{r decile50-figure, fig.cap = "Comparison of the cutoffs for 5th decile across heterogeneity (columns), powering of items (rows), and scale size (color).", fig.height=8, fig.width=8}
summary_long %>% 
  filter(name == "Decile 50") %>% 
  group_by(variability, original_n, scale_size, name, power) %>% 
  summarize(new_sample = mean(new_sample)) %>% 
  ...
  facet_grid(power~ variability,
             labeller = labeller(power = power.labs)) + 
  ggtitle("Results for 5th decile") +
  xlab("\nPilot Sample Size") + 
  ylab("Corrected Sample Size\n") + 
...
```{r bimodal-figure, fig.cap = "Simulated example of Likert data with half of the items as bimodal distributions.", fig.height=10, fig.width=7, warning = F, include = F, message = F}
