...
```{r decile40-figure, fig.cap = "Comparison of the cutoffs for 40% deciles across heterogeneity (columns), powering of items (rows), and scale size (color).", fig.height=8, fig.width=8}
summary_long %>% 
...
```{r decile50-figure, fig.cap = "Comparison of the cutoffs for 50% deciles across heterogeneity (columns), powering of items (rows), and scale size (color).", fig.height=8, fig.width=8}
summary_long %>% 
  filter(name == "Decile 50") %>% 
  group_by(variability, original_n, scale_size, name, power) %>% 
  summarize(new_sample = mean(new_sample)) %>% 
  ...
  facet_grid(power~ variability,
             labeller = labeller(power = power.labs)) + 
  ggtitle("Results for 50% deciles") +
  xlab("\nPilot Sample Size") + 
  ylab("Corrected Sample Size\n") + 
