...
```{r eval = F}
all_sim <- bind_rows(
  bind_rows(readRDS("simulation_linux4_0815.Rdata")), 
  bind_rows(readRDS("simulation_linux8_0815.Rdata")), 
  bind_rows(readRDS("simulation_linux8_0816.Rdata")),
  bind_rows(readRDS("simulation_linux36_0819.Rdata")),
  bind_rows(readRDS("simulation_windows3_0815.Rdata")), 
  bind_rows(readRDS("simulation_windows39_0819.Rdata")),
  bind_rows(readRDS("simulation_mac_0815.Rdata")), 
  bind_rows(readRDS("simulation_mac_0817.Rdata"))
) %>% unique()

summary_long <- all_sim %>% 
  group_by(scale, original_n, source, name, power, scale2) %>% 
  summarize(
    n = n(),
...
```{r}
export(summary_long, "simulated_summary_data_update.csv", row.names = F)
```
