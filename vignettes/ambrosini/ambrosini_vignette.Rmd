...
Our minimum sample size is small at 80% (*n* = `r final_table %>% filter(percent_below >= 80) %>% arrange(percent_below, corrected_sample_size) %>% slice_head() %>%  pull(corrected_sample_size) %>% round()` as the minimum). We could consider using 90% (*n* = `r final_table %>% filter(percent_below >= 90) %>% arrange(percent_below, corrected_sample_size) %>% slice_head() %>%  pull(corrected_sample_size) %>% round()`) or 95% `r final_table %>% filter(percent_below >= 95) %>% arrange(percent_below, corrected_sample_size) %>% slice_head() %>%  pull(corrected_sample_size) %>% round()`. 
