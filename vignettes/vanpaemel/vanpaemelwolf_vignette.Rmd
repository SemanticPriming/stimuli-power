...
```{r}
metadata <- import("vanpaemelwolf_metadata.xlsx")
...
final_sample <- 
  ...
  dplyr::summarize(Percent_Below = sum(se <= cutoff)/length(unique(typicality_data_gp1_sub$comp_group)))  %>% 
  dplyr::filter(Percent_Below >= .80) %>% 
  dplyr::arrange(Percent_Below) %>% 
  mutate(new_sample = round(39.369 + 0.700*sample_size + 0.003*cutoff - 0.694*length(unique(typicality_data_gp1_sub$comp_group))))
...
Notice in this case, because of the smaller number of items, we do find some variability in our estimates, so we selected the min overall for the smallest sample size and the minimum overall above 90% which were the same estimation. 
